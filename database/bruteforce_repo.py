...
class BruteForceRepo(Repo):
    ...
    def build_insert_query(self, entity: BruteForcePersistentStats) -> str:
        command = "INSERT INTO brute_force_module_stats (time_window_start, time_window_end, mean_tries_per_addr) VALUES ('{}','{}', '{}')".format(
            entity.time_window.start, entity.time_window.end, entity.mean_tries_per_addr
        )
        return command

    def build_get_all_query(self, limit=10, offset=0, where_clause='id IS NOT NULL', order='ASC') -> str:
        command = "SELECT id, time_window_start, time_window_end, mean_tries_per_addr FROM brute_force_module_stats WHERE {} ORDER BY time_window_start {} LIMIT {} OFFSET {}".format(
    ...
    def entity_from_tuple(self, tuple: Tuple) -> BruteForcePersistentStats:
        ...
        return BruteForcePersistentStats(id, TimeWindow(time_window_start, time_window_end), mean_tries_per_addr)
